...
class Controller:
    ...
    async def list_user(all=None):
        all = service_user.list_all()
        if all is None:
            return JSONResponse(content={'res': 'User not found'})
        return JSONResponse(all)

    @router.post('/', status_code=status.HTTP_201_CREATED)
    async def create_user(request: Request, exception=Exception):
        ...
        try:
            ...
            return JSONResponse(content={'id': new_id, 'name': name},
                                status_code=201)
        except exception:
    ...
    async def get_user(user_id: int):
        ...
        return JSONResponse(content={'id': user.id, 'name': user.name,
    ...
    async def update_user(user_id: int, request: Request):
        ...
        user = User()
        user.name = name if name is not None else ''
        user.password = password if password is not None else ''
        try:
